/**
 * Toolbar control (React) for the Heading extension, which adds headings (h1–h6). Renders the button and dispatches the matching editor command when activated.
 */
...
export function RichTextHeading({ level }: { level?: number } = {}) {
  const { t } = useLocale();
  ...
  if (level !== undefined) {
    const item = items.find((i: any) => i.level === level);
    if (!item) return <></>;
    return (
      <button
        type="button"
        ...
        onClick={item.action}
        className="richtext-h-[32px] richtext-px-2 richtext-text-sm"
      >
        {item.title}
      </button>
    );
