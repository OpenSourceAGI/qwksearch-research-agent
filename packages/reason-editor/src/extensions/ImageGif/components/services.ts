/**
 * Service helpers for querying the Giphy API (search and trending GIFs). Feeds results into the GIF picker of the ImageGif extension.
 */
...
export async function serviceGetTrendingGiphy(apiKey: string): Promise<GifItem[]> {
  const res = await grab<IResGiphy>(
    `https://api.giphy.com/v1/gifs/trending?q=&limit=15&api_key=${apiKey}`
  );
  const r = res.data as IResGiphy;
...
export async function serviceSearchGiphy(term: string, apiKey: string): Promise<GifItem[]> {
  const res = await grab<IResGiphy>(
    `https://api.giphy.com/v1/gifs/search?q=${term}&limit=15&api_key=${apiKey}`
  );
  const r = res.data as IResGiphy;
...
export async function serviceGetTrendingTenor(apiKey: string): Promise<GifItem[]> {
  const resTrend = await grab<{ results: string[] }>(
    `https://tenor.googleapis.com/v2/trending_terms?key=${apiKey}&limit=10`
  );
  const rTrend = resTrend.data as { results?: string[] };

  const res = await grab<IResTenor>(
    `https://tenor.googleapis.com/v2/search?key=${apiKey}&q=${rTrend?.results?.[0]}&limit=15`
  );
  const r = res.data as IResTenor;
...
export async function serviceSearchTenor(term: string, apiKey: string): Promise<GifItem[]> {
  const res = await grab<IResTenor>(
    `https://tenor.googleapis.com/v2/search?key=${apiKey}&q=${term}&limit=15`
  );
  const r = res.data as IResTenor;
