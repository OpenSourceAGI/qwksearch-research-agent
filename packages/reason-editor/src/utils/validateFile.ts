/**
 * Validates uploaded files against allowed types and size limits. Used by upload flows to reject invalid media before it is inserted.
 */
...
function isAcceptedMime(file: File, acceptMimes?: string[]): boolean {
  // No accept list configured -> accept any file type.
  if (!acceptMimes || acceptMimes.length === 0) return true;
