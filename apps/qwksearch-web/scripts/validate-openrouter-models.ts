...
/**
 * CLI script to validate OpenRouter free models
 *
 * Usage:
 *   tsx apps/qwksearch-web/scripts/validate-openrouter-models.ts
 *   tsx apps/qwksearch-web/scripts/validate-openrouter-models.ts --concurrency 5 --timeout 20000
 *
 * Options:
...
async function main() {
  ...
  console.log("=== OpenRouter Free Models Validator ===\n");

  if (!process.env.OPENROUTER_API_KEY) {
    console.error("ERROR: OPENROUTER_API_KEY not found in .env file");
    console.error(
      "Please add your OpenRouter API key to apps/qwksearch-web/.env"
    );
    process.exit(1);
  }
