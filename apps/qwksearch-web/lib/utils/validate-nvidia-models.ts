/**
 * @fileoverview NVIDIA Cloud Free Models Validator
 *
 * Validates which NVIDIA cloud free models actually work with the configured API key.
 * Only models that pass validation should be shown to guests and new users.
 *
 * Usage:
 *   import { validateNvidiaModels, getValidatedFreeModels } from '@/lib/utils/validate-nvidia-models';
 *
...
export interface NvidiaValidationResult {
  totalTested: number;
...
/**
 * Get NVIDIA free models from the database
 */
function getNvidiaFreeModels() {
  const nvidiaProvider = LANGUAGE_MODELS.find(
    (p) => p.provider.toLowerCase() === "nvidia"
  );

  if (!nvidiaProvider) {
    throw new Error("NVIDIA provider not found in database");
  }

  return nvidiaProvider.models.filter((m) => m.free === true);
}

/**
 * Test a single NVIDIA model using native fetch API
 */
async function testNvidiaModel(
  ...
  try {
    ...
    // Test with a minimal prompt using OpenAI-compatible API
...
/**
 * Validate all NVIDIA free models with the configured API key
 *
 ...
 */
export async function validateNvidiaModels(
  concurrency: number = 3,
  timeout: number = 15000,
  onProgress?: (current: number, total: number, modelName: string) => void
): Promise<NvidiaValidationResult> {
  const startTime = Date.now();
  ...
  const freeModels = getNvidiaFreeModels();
  ...
  const workers = Array(Math.min(concurrency, freeModels.length))
    ...
    .map(async () => {
      while (queue.length > 0) {
        ...
        const result = await testNvidiaModel(
          apiKey,
          model.id,
          model.name,
          timeout
  ...
  console.log("\n=== NVIDIA Cloud Free Models Validation Summary ===");
