...
12. Root `bun run test` (Vitest workspace) has 29 failing test files / 74 failing tests, all under `apps/qwksearch-web` (e.g. `lib/notebooklm/__tests__/credentials.test.ts`, `app/api/config/__tests__/route.test.ts`), while `cd apps/qwksearch-web && bun run test` passes the same files — points to a root-vs-local Vitest version/config mismatch (root uses vitest `^4.0.18`, `apps/qwksearch-web` pins `^3.2.0`) rather than a bug in the tests themselves. Follow-up to #40.
